...
7. [test/http/test.http](test/http/test.http) 统一维护可直接执行的 HTTP 接口完整正常请求示例，不添加参数错误、非法请求等失败场景。新增接口时必须同步补充对应请求；接口的方法、路径、请求头、请求参数或响应形式发生变化时，必须同步更新相关请求。未同步维护该文件时，HTTP 接口变更视为未完成。
