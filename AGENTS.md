...
4. 设计和实现代码时，必须遵循高内聚、低耦合和单一职责等设计原则，同时考虑可扩展性、可维护性和代码可读性；优先使用清晰、简单、职责明确的设计，避免过度抽象、重复实现和不必要的耦合。
5. 代码注释统一使用中文。关键业务规则、边界条件、并发控制、错误处理、安全限制、兼容性处理以及不直观的设计决策必须写明注释；注释应重点解释“为什么”和约束条件，不机械复述代码。
6. 每次完成代码变更后，必须显式执行 `go build ./...` 并确认编译通过；不得仅以测试通过替代编译检查。编译失败时变更视为未完成，必须修复后才能交付。
