...
│   ├── browser/                   # 浏览器执行端（Chrome 扩展 lead-mind-ai-plugin）对接，已实现
│   │   ├── protocol.go            # ★★ 跨仓库契约，对应插件 src/shared/protocol.ts，改动需两端同步
│   │   ├── codec.go               # 信封编解码（不信任对端，坏帧丢弃不断连）
│   │   ├── hub.go                 # 连接注册表（tenant_code+device_id 为键）、认证握手、帧分发
│   │   ├── conn.go                # 单连接：并发写保护、指令下发与 result 配对、审批超时延长
│   │   ├── auth.go                # device_token 校验（区分凭证失效与内部故障）
│   │   ├── dispatch.go            # 工具层入口：从 context identity 解析目标设备
│   │   ├── handler.go             # 配对/WS 接入/设备管理 HTTP 端点
│   │   └── device/                # 设备身份：配对码(Redis 一次性) + 设备表(租户 MySQL)
...
│   ├── tools/
│   │   ├── browser/               # 浏览器指令的 Eino 工具封装（已实现，14 个工具）
...
| 2026-07-27 | 落地浏览器执行端（Chrome 扩展 `lead-mind-ai-plugin`）的服务端对接：新增 `internal/browser`（`protocol.go` 跨仓库契约 + `hub.go` 连接注册表 + `conn.go` 单连接指令往返 + `auth.go` 凭证校验 + `dispatch.go` 工具入口 + `handler.go` HTTP 端点）、`internal/browser/device`（配对码 Redis 一次性存储 + 设备表落租户 MySQL）、`internal/tools/browser`（把 16 条浏览器指令封装成 14 个 Eino 工具挂到 ReAct Agent）；新增迁移 `migrations/0003_create_browser_devices.sql`；新增路由 `POST /ai-agent/v1/device/pair`、`GET /ai-agent/v1/device/connect`（WS）、`POST /ai-agent/v1/device/pairing-code`、`GET/DELETE /ai-agent/v1/devices`；新增直接依赖 `github.com/coder/websocket`。**关键设计取舍**：(1) **指令下发的目标设备只由 `identity.FromContext` 决定，工具无法指定**——模型被 prompt injection 劫持后也无法把指令发到别的租户/用户的浏览器，这是 §6.2 红线在本模块的落点，注册表以 `(tenant_code, device_id)` 为键并复核 `user_id` 归属；(2) **指令级失败作为正常返回值交给模型，而非 Go error**——Eino 里工具返回 error 会中断 ReAct 循环，而"元素不见了/需要重读页面"恰恰是模型该看到并据此调整的信息，只有传输层失败（设备离线、超时）才返回 error；(3) **`ack{pending_approval}` 必须能延长超时**——高危指令在插件侧要等真人点确认（可能几十秒），不延长的话所有写操作都会在用户看到弹窗前就被判超时；(4) **服务端的 `risk` 标注不是安全边界**，插件用本地硬编码表做最终判定，服务端标注只用于交叉校验以暴露契约漂移；(5) **`device_token` 落库存 SHA-256 而非明文**，明文只在配对响应里出现一次。实现中发现并修复了两个隐患：一是 `middleware.Logging` 的 `responseRecorder` 没有实现 `http.Hijacker`，会让 `websocket.Accept` 直接失败（包装 `ResponseWriter` 时丢掉底层可选接口的经典陷阱），已补 `Hijack` 方法；二是 `Hub.Disconnect` 原先依赖 `readLoop` 结束后才摘除注册表，而 WebSocket 的 close 握手在对端不配合时最长等 5 秒，这几秒里被吊销的设备仍能被 `Lookup` 命中并接收指令——改为**同步摘除注册表、异步关闭连接**，使吊销立刻生效。`go build`/`go vet`/`gofmt`/`go test ./... -race` 全绿；新增 47 个测试，其中 `integration_test.go` 用 httptest + 真实 WebSocket 客户端扮演 mock 插件，覆盖了认证握手成功/坏 token/已吊销/已过期、未认证连接发非 auth 帧被拒、指令完整往返、跨租户下发被拒、能力协商拦截、`ack` 延长审批超时（对照组：无 ack 时基础超时正常生效）、心跳 ping/pong、畸形帧不断连、吊销立刻摘除注册表。**未做真实环境验证**：沙箱无 Redis/MySQL，配对码的 Redis `SETNX`/`GETDEL` 与设备表的 MySQL 读写未做集成验证；也未与真实 Chrome 扩展联调（插件 README 亦记录其"未做真机联调"），落地前建议在有 Redis+MySQL 的环境加载插件跑一次完整配对→连接→指令链路 |
