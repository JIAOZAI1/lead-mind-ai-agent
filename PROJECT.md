...
- **隔离级别**：租户间共享基础设施，隔离主要依赖**逻辑层隔离**（`tenant_id` 贯穿所有表和缓存 key），不做独立 schema/独立部署（除非未来出现企业级客户，届时再评估）。
...
贯穿：可观测性(OTel) · 审批网关(HITL) · 多租户隔离(tenant_id) · 密钥管理
```
...
1. **`tenant_id` 贯穿一切**：所有 MySQL 表（除全局配置表）必须有 `tenant_id` 列并建索引；所有 Redis key 必须以 `tenant:{tenant_id}:` 为前缀；所有日志/trace 必须打 `tenant_id` 标签。
2. **逻辑隔离，非物理隔离**：MVP 阶段不做独立 schema/独立数据库。查询层要通过统一的 repository 封装强制带 `tenant_id` 过滤，杜绝手写 SQL 漏加租户条件导致数据串租户。
3. **配额与限流按租户维度**：模型调用次数/token 消耗、工具调用频率、Agent 并发数都要有按租户的限流（Redis 计数器），避免单租户耗尽共享资源影响其他租户。
4. **计费预留字段**：`tenants` 表设计时预留套餐等级（`tier`）、计费周期、用量统计字段，即使 MVP 阶段先手动开通，也要让数据模型未来能接自动化计费。
...
**阶段二：多租户与可观测性（1~2 周）**
补齐 `tenant_id` 贯穿、租户配额限流、Callback → OTel trace/metrics。这一步优先级极高——没有多租户隔离就不能叫 SaaS，没有可观测性后续所有优化都是盲人摸象。
...
- 任何新增的数据库查询、缓存读写，Code Review 时必须检查是否带 `tenant_id` 过滤，这是本项目最高优先级的安全红线（对标参考文档 §6 的"越权拦截"）。
